...
pub fn list_refs(cwd: &Path) -> Result<Vec<GitRef>> {
    ...
    let result = run_git_with(
        cwd,
        vec!["for-each-ref".into(), format!("--format={format}"), "refs/heads".into(), "refs/remotes".into()],
        RunOptions::default(),
    )?;
...
pub fn parse_refs(buffer: &[u8]) -> Vec<GitRef> {
    ...
    while index + 2 < values.len() {
        let full_name = values[index].trim_start_matches(|character| character == '\r' || character == '\n').to_owned();
        let oid = values[index + 1].clone();
        let upstream = values[index + 2].trim_end_matches(|character| character == '\r' || character == '\n').to_owned();
        index += 3;
        ...
        if let Some(short_name) = full_name.strip_prefix("refs/heads/") {
            refs.push(GitRef {
                ...
                upstream: if upstream.is_empty() {
                    ...
                    Some(upstream.strip_prefix("refs/remotes/").unwrap_or(&upstream).to_owned())
                },
            });
...
pub fn resolve_oid(cwd: &Path, reference: &str) -> Result<Option<String>> {
    let result = run_git_with(
        cwd,
        vec!["rev-parse".into(), "--verify".into(), format!("{reference}^{{commit}}")],
        RunOptions {
...
mod tests {
    ...
    fn parses_local_and_remote_refs() {
        let input = b"refs/heads/main\0abc\0refs/remotes/origin/main\0\nrefs/remotes/origin/feature/x\0def\0\0";
