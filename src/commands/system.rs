...
pub fn command_system_inspect(context: &CommandContext) -> Result<i32> {
    ...
    for workspace in &state.workspaces {
        ...
        renderer.line(format!(
            "    <dim>{}</dim>",
            renderer.value(workspace.path.display().to_string())
        ));
    }
    renderer.line("");
...
pub fn command_system_doctor(context: &CommandContext) -> Result<i32> {
    ...
    checks.push(DoctorCheck {
        ...
        ok: true,
        detail: config.root.display().to_string(),
    });
            ...
            Ok(state) => {
                let broken = state
                    ...
                    + state
                        .slots
                        .iter()
                        .filter(|slot| slot.status == WorkspaceStatus::Broken)
                        .count();
                let idle = state
                    .slots
                    .iter()
                ...
                checks.push(DoctorCheck {
                    ...
                    ok: true,
                    detail: repository.top_level.display().to_string(),
                });
                checks.push(DoctorCheck {
                    name: "Acre state".into(),
                    ok: broken == 0,
                    detail: if broken == 0 {
                        "consistent".into()
                    } else {
